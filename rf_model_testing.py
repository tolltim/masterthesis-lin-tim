...
def get_config():
    ...
    return {
        ...
        "grid_search": False,
        'road_closure_date': "11.06.2023",  # closure date of südliche au but here it only means t
...
def save_predictions_to_csv(X_test, y_test, y_pred, date_series, filename="predictions/predictions_au.csv"):
    """
    Save the predictions to a CSV file, including the 'date' column for reference.
    """
    # Ensure date_series is properly aligned with X_test
    date_df = pd.DataFrame({'date': date_series}).reset_index(drop=True)

    y_pred_df = pd.DataFrame(y_pred, index=y_test.index, columns=y_test.columns).reset_index(drop=True)
    y_pred_df = y_pred_df.add_prefix('Predicted_')

    y_test_df = y_test.add_prefix('True_').reset_index(drop=True)

    # Concatenate all parts including the date
    results_df = pd.concat([date_df, y_test_df, y_pred_df], axis=1)

    # Check if the required columns are present
    print("DataFrame columns before saving:", results_df.columns)

    # Save to CSV
    results_df.to_csv(filename, index=False)
    print(f"Predictions saved to {filename}")

def evaluate_model(y_test, y_pred, X_test, X_dates):
    """
    ...
    X_test_filtered = X_test[mask]  # Indices are now aligned, so this should work
    ...
    overall_mae = mean_absolute_error(y_test_filtered, y_pred_df)
    overall_r2 = r2_score(y_test_filtered, y_pred_df)
    print(f"\nOverall Model Metrics: MAE = {overall_mae}, R² = {overall_r2}")
    save_predictions_to_csv(X_test, y_test, y_pred, X_dates)
...
def main():
    ...
    evaluate_model(y_test, y_pred, X_test, X_dates.loc[X_test.index])
