...
def get_config():
    ...
    The selected features and target variables are based on the train-data-wp
...
def add_pre_closure_means_by_weekday(merged_data, features, road_closure_date_str): ### right now this is ignored with zero values passed
...
def feature_target_selection(merged_data, road_closure_date, use_all_features, selected_features, targets, test_size, random_state):
    ...
    if use_all_features:
        selected_features_df = pd.read_csv("train-data/variables.csv") ### can not be taken anymore
...
def main():
    ...
    features_to_process = [] # Replace with your actual feature names, right now it is emtpy for perfect prediction model
